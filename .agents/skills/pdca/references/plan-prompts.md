...
> **Tool check:** Before running this analysis, does the coding agent have a tool to explore codebase structure to support pattern discovery? (e.g., file search, glob, grep) Would entering a planning mode help scope this analysis?
...
- [ ] **SEARCH 1**: Search the codebase for similar feature implementations (query: "How does [similar functionality] work in the codebase?")
- [ ] **SEARCH 2**: Search the codebase for integration patterns (query: "Where are [related services/components] integrated with existing systems?")
- [ ] **SEARCH 3**: Search the codebase for configuration patterns (query: "How are similar configuration options implemented and used?")
...
- [ ] Use available tools or direct inspection to validate actual formats/behaviors
...
> **Tool check:** Now that pattern discovery is complete, is there an agent tool to trace how these patterns connect across the codebase?
